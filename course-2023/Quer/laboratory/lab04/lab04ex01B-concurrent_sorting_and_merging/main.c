...
int main(int argc, char **argv) {
    ...
    // Await for termination of all the threads and merge the result into the array
    int *merged;
    merged = (int *)malloc(0*sizeof(int));
    int size = 0;
    for(int i=0; i< n-1; i++){
        pthread_join(t[i],NULL);

        merged = realloc(merged,(size + td[i].len) * sizeof(int));
