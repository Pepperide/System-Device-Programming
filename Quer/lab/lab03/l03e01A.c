...
int main(){
	...
	//Starting pipe for C1 and C2
	if( pipe(p1) == 0 && pipe(p2)==0){
		
		pid1=fork(); //Generating of C1
		if(pid1==-1){
		...
		if(pid1==0){
			...
			while(count<=NUM_STR){
				...
				write(p1[1],s,STR_SIZE*sizeof(char));
				free(s);
			...
			pid2=fork(); //Generating C2 from P
			if(pid2==-1){
				fprintf(stderr,"Fork failure");
				exit(EXIT_FAILURE);
			}
			...
			if(pid2==0){
				...
				close(p2[0]); //Closing the read-end. C1 onli write on p1
				while(count<=NUM_STR){
					sleep(WAIT_TIME_2);
					char *s=(char*)malloc(STR_SIZE*sizeof(char));
					randomString(s,STR_SIZE);
					write(p2[1],s,STR_SIZE*sizeof(char));
					free(s);
				...
				for(int i=0;i<NUM_STR;i++){
					...
					read(p1[0],s,STR_SIZE*sizeof(char));
					capitalize(s);
					printf("String from C1: %s\n",s);
					
					read(p2[0],s,STR_SIZE*sizeof(char));
					capitalize(s);
					printf("String from C2: %s\n",s);
